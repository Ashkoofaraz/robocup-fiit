...
/**
 * Undefined StateTactic is implemented for undefined state in all tactics
 * because our robot isn't dummy
 *
...
public class UndefinedStateTactic
{
    ...
    public void run(List<String> currentSituations)
    {
        ...
        {
            this.moveExec.move(
                    Vector3D.cartesian((ballX + AGENT_OFFSET_TO_BALL_X), this.getDestinationY(agentY, ballY), 0),
        ...
        else
        {
            if(currentSituations.contains(BallNearestToMe.class.getName()))
            {
                ...
                {
                    this.moveExec.move(Vector3D.cartesian(ballX, ballY, 0), MovementSpeedEnum.WALK_FAST);
                }
                else
                {
                    this.moveExec.move(Vector3D.cartesian(ballX, ballY, 0), MovementSpeedEnum.WALK_MEDIUM);
                }
            }
            else
            {
    ...
    protected double getDestinationY(double agentY, double ballY)
    {
        ...
        double positionY = (agentY + (agentY > ballY ? (-Defend.AGENT_OFFSET_TO_BALL) : Defend.AGENT_OFFSET_TO_BALL));
