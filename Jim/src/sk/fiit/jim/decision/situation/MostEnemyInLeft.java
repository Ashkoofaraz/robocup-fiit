...
public class MostEnemyInLeft extends Situation {
	...
	public boolean checkSituation() {
		for (Player p : opponents) {
			if ( p.getPosition().getY() < MostEnemyInLeft.POSITION_Y_RIGHT && 
					p.getPosition().getY() > MostEnemyInLeft.POSITION_MAX_Y_RIGHT) {
			count_right++;	
			}
			if ( p.getPosition().getY() > MostEnemyInLeft.POSITION_Y_LEFT && 
					p.getPosition().getY() < MostEnemyInLeft.POSITION_MAX_Y_LEFT) {
			count_left++;	
			}
			if ( p.getPosition().getY() > MostEnemyInLeft.POSITION_Y_RIGHT && 
					p.getPosition().getY() < MostEnemyInLeft.POSITION_Y_LEFT) {
			count_mid++;	
			}
		}
		
		if(count_left> count_right && count_left> count_mid){return true;}		
		return false;
	}
}