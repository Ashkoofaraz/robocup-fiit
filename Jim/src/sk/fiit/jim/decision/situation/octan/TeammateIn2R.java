 ...
 *         Our
 *         
 * @return boolean - Returns true only if teammate is in octan R2
 */

public class TeammateIn2R extends Situation {
	...
	public boolean checkSituation() {
		for (Player p : teammates) {
			if (p.getPosition().getX() > TeammateIn2R.POSITION_MIN_X
					&& p.getPosition().getX() < TeammateIn2R.POSITION_MAX_X
					&& p.getPosition().getY() < TeammateIn2R.POSITION_MAX_Y) {
				return true;
			}
		}
		return false;
