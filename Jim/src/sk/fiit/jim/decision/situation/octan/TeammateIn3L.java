 ...
 *         Our
 *         
 * @return boolean - Returns true only if teammate is in octan L3
 */

public class TeammateIn3L extends Situation {
	...
	public boolean checkSituation() {
		for (Player p : teammates) {
			if (p.getPosition().getX() > TeammateIn3L.POSITION_MIN_X
					&& p.getPosition().getX() < TeammateIn3L.POSITION_MAX_X
					&& p.getPosition().getY() > TeammateIn3L.POSITION_MIN_Y) {
				return true;
			}
		}
		return false;
