...
public class BallIn1L extends Situation {
	...
	public boolean checkSituation() {
		...
		if ((WorldModel.getInstance().getBall().getPosition().getX() < BallIn1L.POSITION_MAX_X)
				&& (WorldModel.getInstance().getBall().getPosition().getY() > BallIn1L.POSITION_MIN_Y)) {
			return true;
		}
		return false;
	}
