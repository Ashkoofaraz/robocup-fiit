 ...
 *         Our
 *         
 * @return boolean - Returns true only if teammate is in octan R3
 */

public class TeammateIn3R extends Situation {
	...
	public boolean checkSituation() {
		for (Player p : teammates) {
			if (p.getPosition().getX() > TeammateIn3R.POSITION_MIN_X
					&& p.getPosition().getX() < TeammateIn3R.POSITION_MAX_X
					&& p.getPosition().getY() < TeammateIn3R.POSITION_MAX_Y) {
				return true;
			}
		}
		return false;
