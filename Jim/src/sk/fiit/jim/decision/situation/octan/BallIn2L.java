...
public class BallIn2L extends Situation {
	...
	public boolean checkSituation() {
		if ((WorldModel.getInstance().getBall().getPosition().getX() <= BallIn2L.POSITION_MAX_X)
				&& (WorldModel.getInstance().getBall().getPosition().getX() >= BallIn2L.POSITION_MIN_X)
				&& (WorldModel.getInstance().getBall().getPosition().getY() >= BallIn2L.POSITION_MIN_Y)) 
		{
			return true;
		}
		return false;
