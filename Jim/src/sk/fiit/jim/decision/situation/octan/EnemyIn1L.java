 ...
 *         Our
 *         
 * @return boolean - Returns true only if enemy is in octan L1
 */

public class EnemyIn1L extends Situation {
	...
	public boolean checkSituation() {
		for (Player p : opponents) {
			if (p.getPosition().getX() < EnemyIn1L.POSITION_MAX_X
					&& p.getPosition().getY() > EnemyIn1L.POSITION_MIN_Y) {
				return true;
			}
		}
		return false;
