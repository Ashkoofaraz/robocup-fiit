...
public class BallIn3R extends Situation {
	...
	public boolean checkSituation() {
		if ((WorldModel.getInstance().getBall().getPosition().getX() > BallIn3R.POSITION_MIN_X)
				&& (WorldModel.getInstance().getBall().getPosition().getX() <= BallIn3R.POSITION_MAX_X)
				&& (WorldModel.getInstance().getBall().getPosition().getY() < BallIn3R.POSITION_MAX_Y)) {
			return true;
		}
		return false;
	}
