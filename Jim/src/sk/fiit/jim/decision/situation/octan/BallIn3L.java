...
public class BallIn3L extends Situation {
	...
	public boolean checkSituation() {
		if ((WorldModel.getInstance().getBall().getPosition().getX() > BallIn3L.POSITION_MIN_X)
				&& (WorldModel.getInstance().getBall().getPosition().getX() <= BallIn3L.POSITION_MAX_X)
				&& (WorldModel.getInstance().getBall().getPosition().getY() >= BallIn3L.POSITION_MIN_Y)) {
			return true;
		}
		return false;
	}
