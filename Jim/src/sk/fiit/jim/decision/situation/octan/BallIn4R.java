...
public class BallIn4R extends Situation {
	
	public static final double POSITION_MIN_X = 8;
	public static final double POSITION_MAX_Y = 0;
	
	public boolean checkSituation() {
		if ((WorldModel.getInstance().getBall().getPosition().getX() > BallIn4R.POSITION_MIN_X)
				&& (WorldModel.getInstance().getBall().getPosition().getY() < BallIn4R.POSITION_MAX_Y)) {
			return true;
		}
		return false;
	}
