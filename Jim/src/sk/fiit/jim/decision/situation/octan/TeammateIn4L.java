 ...
 *         Our
 *         
 * @return boolean - Returns true only if teammate is in octan L4
 */

public class TeammateIn4L extends Situation {
	...
	public boolean checkSituation() {
		for (Player p : teammates) {
			if ((p.getPosition().getX() > TeammateIn4L.POSITION_MIN_X)
					&& (p.getPosition().getY() > TeammateIn4L.POSITION_MIN_Y)) {
				return true;
			}
		}
		return false;
