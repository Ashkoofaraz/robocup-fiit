 ...
 *         Our
 *         
 * @return boolean - Returns true only if enemy is in octan R2
 */

public class EnemyIn2R extends Situation {

	public static final double POSITION_MIN_X = -7.5;
	...
	public boolean checkSituation() {
		for (Player p : opponents) {
			if (p.getPosition().getX() > EnemyIn2R.POSITION_MIN_X
					&& p.getPosition().getX() < EnemyIn2R.POSITION_MAX_X
					&& p.getPosition().getY() <= EnemyIn2R.POSITION_MAX_Y) {
				return true;
			}
		}
		return false;
