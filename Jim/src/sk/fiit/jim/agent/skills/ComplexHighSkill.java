...
public abstract class ComplexHighSkill extends HighSkill implements IComplexHighSkill
{
    ...
    public LowSkill pickLowSkill()
    {
        ...
        // EXEC -> try to get HighSkill and return its LowSkill only if returned
        // LowSkill from actual HighSkill is null
        switch (state)
        {
        case INIT:
    ...
    private LowSkill getLowSkillFromNewHighSkill()
    {
        ...
        if(hs == null)
        {
            // If returned highSkill is null, then complex high skill is at the
            // end
            // This is how does it works -> if you want to end complex high
            // skill,
            // just return null in pickHighSkill method -> null LowSkill will be
            // returned
