 ...
 * 
 * <p>
 * HighSkill represents an action - such as walking to a point, turning or
 * standing up. This is achieved by selecting the appropriate low skills to
 * perform smaller parts of the action.
 * </p>
 * <p>
 * High skills are implemented by subclassing this class. Most of them are
 * currently written in Ruby and placed in the scripts/high_skills directory.
 * When subclassing a high skill, the following methods need to be implemented:
 * <ol>
 ...
 * </p>
 * <p>
 * For details on how to implement a new high skill, see (TODO: wiki page)
 * </p>
 *
 *
 * @Title Jim
...
public abstract class HighSkill implements IHighSkill
{
    ...
    /**
     * Low skill that was returned by {@link #pickLowSkill}, but is not yet
     * active, because the current skill needs to be finalized first. Once that
     * happens, this skill will become the current low skill (or the high skill
     * will end if nextSkill is null).
     */
    ...
    /**
     * Contains the possible states that a high skill can be in. These affect
     * the behavior of the {@link HighSkill#execute} method.
     * <ul>
     * <li>INITIAL_STATE - the starting state of all high skills</li>
     * <li>EXECUTING_STATE - the high skill is currently executing a chosen low
     * skill</li>
     * <li>FINALIZING_STATE - the high skill is currently bringing the chosen
     * low skill into a safe, final state. Once that's done, a new low skill
     * will be executed, or the high skill will end (depending on whether
     * nextSkill is null)</li>
     * <li>END_STATE - the high skill has ended and can no longer be used</li>
     * </ul>
     */
    public enum HighSkillState
    {
        INITIAL_STATE, EXECUTING_STATE, FINALIZING_STATE, END_STATE
    };

    /**
     * The state that the high skill is currently in. See {@link HighSkillState}
     * for details.
     */
    ...
    /**
     * <p>
     * Performs a single step of the high skill. This means handling the
     * selection of low skills, their execution and finalization. For details,
     * see TODO: wiki link
     * </p>
     * <p>
     * This method can also be subclassed to implement a more complex control
     * structure (high skills that execute other high skills, etc.)
     * </p>
     *
     * @throws Exception
     */
    public final void execute() throws Exception
    {
        ...
        {
        case INITIAL_STATE:
            // get a low skill from the specific high skill (usually a ruby
            // script)
            ...
            if(currentSkill.canFinalize() && !isStoppedHighSkill())
            {
                ...
                if(nextSkill == currentSkill)
                {
                    ...
                    // so it is possible to connect them directly
                        ...
                        && Phases.areSimilarPhases(Phases.get(nextSkill.initialPhase), currentSkill.activePhase, 100))
                {
    ...
    /**
     * addeded by Bimbo (High5), communication with test framework Blank
     * try-catch because of ruby BUG - have to be in Lowskill
     */
    private static void sayMoveStoppedToFramwork(LowSkill lowSkill)
    {
        try
        {
            ...
            TestFrameworkCommunication.sendMessage(new Message().HighSkill().Stop(lowSkill.name,
                    AgentModel.getInstance().getLastDataReceived().GAME_TIME));
        }
        catch (Exception e)
        {
        }
    ...
    private static void sayMoveStartedToFramwork(LowSkill lowSkill)
    {
        try
        {
            ...
            TestFrameworkCommunication.sendMessage(new Message().HighSkill().Start(lowSkill.name,
                    AgentModel.getInstance().getLastDataReceived().GAME_TIME));
        }
        catch (Exception e)
        {
    ...
    /**
     * <p>
     * Called everytime the current low skill enters a phase that be can be
     * finalized (so that it's possible to safely switch to a new low skill, see
     * TODO: wiki link)
     * </p>
     * <p>
     * Returns the next skill that needs to be executed in order to fulfill
     * goals of this HighSkill.
     * </p>
     * <p>
     * If the returned skill is the same as the current skill, it will continue
     * normally.
     * </p>
     * <p>
     * If a different low skill or null is returned, the current low skill will
     * be finalized. Once that finalization has been succesfully completed, the
     * returned low skill will be set as the new executed low skill, or, if null
     * was returned, the high skill will end.
     * 
     * @return The next skill that needs to be executed. Null if the high skill
     *         should stop.
     */
     ...
     * </p>
     * <p>
     * Should throw an exception if the agent is found to be in an unexpected
     * state, e.g. fallen on ground during walking.
     * </p>
     * <p>
     * Causes this skill's execution to be immediately terminated.
     * </p>
     * TODO: does it work? should it work this way? currently, falling is
     * handled by the pickLowSkill methods in our high skills
     * 
     * @throws Exception
     *             means that the high skill needs to be interrupted without
     *             regard to safe finalization of the current low skill
     */
