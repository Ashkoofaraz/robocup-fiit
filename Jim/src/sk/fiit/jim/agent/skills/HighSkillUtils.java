...
public class HighSkillUtils
{
    ...
    public static boolean isOnSideAndClose(Vector3D targetPosition, double strafe_distance, Angles range,
            double targetPositionPhi)
    {
        return (targetPosition.getR() <= strafe_distance) && range.include(Angles.normalize(targetPositionPhi));
    }

    public static boolean isBackAndClose(Angles back_range, double targetPositionPhi, Vector3D targetPosition,
