        ...
        for(int le1 = (int)Joint.LAE1.getLow(); le1 <= (int) Joint.LAE1.getUp(); le1++)
        {
            for(int le2 = (int)Joint.LAE2.getLow(); le2 <= (int) Joint.LAE2.getUp(); le2++)
            {
                for(int le3 = (int)Joint.LAE3.getLow(); le3 <= (int) Joint.LAE3.getUp(); le3++)
                {
                    ...
                    {
                        br.append(new ForwardKinematicResult(kin.getForwardLeftHand(Math.toRadians(le1), Math.toRadians(le2), Math.toRadians(le3), Math.toRadians(le4))).toString() + '\n');
                        //System.out.println(new ForwardKinematicResult(kin.getForwardLeftHand(Math.toRadians(le1), Math.toRadians(le2), Math.toRadians(le3), Math.toRadians(le4))));
                    }
                }
            }
        }
