...
public enum Joint implements Serializable {
    HE1(-120.0, 120.0, Vector3D.ZERO_VECTOR, Vector3D.Z_AXIS), HE2(-45.0, 45.0,
            Vector3D.cartesian(0, 0, -0.005), Vector3D.X_AXIS),

    RLE1(-90.0, 1.0, Vector3D.ZERO_VECTOR, Vector3D.cartesian(-0.7071, 0,
            0.7071)), RLE2(-45.0, 25.0, Vector3D.ZERO_VECTOR, Vector3D.Y_AXIS), RLE3(
            -25.0, 100.0, Vector3D.cartesian(0, -0.01, 0.04), Vector3D.X_AXIS), RLE4(
            -130.0, 1.0, Vector3D.cartesian(0, -0.01, 0.045), Vector3D.X_AXIS), RLE5(
            -45.0, 75.0, Vector3D.ZERO_VECTOR, Vector3D.X_AXIS), RLE6(-25.0,
    ...
    LLE1(-90.0, 1.0, Vector3D.ZERO_VECTOR, Vector3D.cartesian(-0.7071, 0,
            ...
            -25.0, 100.0, Vector3D.cartesian(0, -0.01, 0.04), Vector3D.X_AXIS), LLE4(
            -130.0, 1.0, Vector3D.cartesian(0, -0.01, 0.045), Vector3D.X_AXIS), LLE5(
            -45.0, 75.0, Vector3D.ZERO_VECTOR, Vector3D.X_AXIS), LLE6(-45.0,
    ...
    private Joint(double low, double up, Vector3D anchor, Vector3D axis) {
        this.lowerLimit = low;
        this.upperLimit = up;
        this.anchor = anchor;
        this.axis = axis;
