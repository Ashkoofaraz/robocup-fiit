...
class PlanTournamentKickAccuracy < Plan
  ...
  def replan
    ...
    @kick_target = @mathModel.cartesian(10.5,7.0,0.0) #@agentInfo.kickTarget()
    ...
    @kick_target_phi = diff_against_current #@kick_target.getPhi()
     ...
     if EnvironmentModel.beamablePlayMode and not EnvironmentModel.isKickOffLeftPlayMode
    	...
    	@plan << Beam.new(Java::sk.fiit.robocup.library.geometry.Vector3D.cartesian(-0.3, 0, 0.4))
    	@beamed = true
